...
C {devices/gnd.sym} -160 60 0 1 {name=l3 lab=GND}
...
.control
  run
  plot db(v(vout)) title 'Bode Magnitude (dB)'
  plot (180/pi)*cph(v(vout)) title 'Phase Margin (Degrees)'
.endc"}
C {opamp_two_stage.sym} -10 0 0 0 {name=x1}
C {devices/vsource.sym} -160 30 0 1 {name=V1 value="dc 0 ac 1" savecurrent=false}
